...
export function observeAuthorizationChanges(
  callback: (newState: AuthorizationState) => void
): EmitterSubscription {
  // Add a listener for the 'authorizationChange' event
  addAuthorizationObserver();
  const subscription = eventEmitter.addListener(
    'AuthorizationStatusChange',
    (event) => {
      callback(
        AuthorizationState[event.state as keyof typeof AuthorizationState]
      );
    }
  );
